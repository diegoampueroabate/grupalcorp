...
from .media_handler import download_telegram_photo
...
class TelegramBot:
    ...
    async def _handle_photo(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_text("Imagen recibida y guardada. La usare cuando crees un anuncio.")

        if update.message.caption:
            await context.bot.send_chat_action(chat_id=chat_id, action="typing")
            response = await self.agent.process_message(
                chat_id, f"[El usuario envio una imagen para usar en un anuncio] {update.message.caption}"
            )
